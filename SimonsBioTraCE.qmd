...
Since then, we've experienced a 3-year megadrought, multiple heatwaves and extreme heavy rainfall and flooding events, and aseasonal fires are becoming increasingly common.
...
What is happening to the rest of the thousands of less conspicuous species we simply don't know. Neither do we know the implications for Nature's Contributions to People like carbon storage, water provision or future fire regimes.
:::
...
We need coherent integrative theory that facilitates prediction of biodiversity dynamics and ideally allows iterative learning from new observations.
:::
...
- For example, the Cape Floristic Region is home to >9000 vascular plant species and many more that are yet to be described. We can't hope to directly model every single one of them.
...
In short, we simply haven't had the tools available to build a coherent body of integrative theory for understanding and forecasting biodiversity and its contributions to people.
:::
...
## BioTraCE {.center background-image="img/NG/Ischyrolepis tenuissima_Female.jpg" background-size="300px" background-position="left"}
...
:::{.notes}
Members of our team have been leading efforts to map plant traits from imaging spectroscopy (hyperspectral remote sensing) groundtruthed with field data.
...
To prepare for regular acquisition of hyperspectral satellite data with continuous coverage, we'll use field data and trait maps from airborne campaigns to calibrate and develop trait time-series from decades of existing multispectral satellite records, such as Landsat and Sentinel.
:::
...
:::{.notes}
We can then develop iterative forecasts of these traits that update as new imagery are acquired and processed - which is on the scale of days to weeks. 
...
:::{.notes}
We'll also apply a different approach, leveraging the trait time series and knowledge gained from the forecasts and causal models in a Trait Driver Theory framework.
...
A coherent body of theory and tools for iterative forecasting of biodiversity and Nature's Contributions to People to inform decision making
...
![](img/silvermine_enhanced.jpg){width="90%" height="90%" fig-align="center"}
[**BioTraCE**: A Unified Framework for Understanding and Predicting Biodiversity Change]{style="text-align: center; font-size: 0.8em;"}
